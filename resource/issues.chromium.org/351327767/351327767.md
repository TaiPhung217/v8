Title: WebAssembly OOB memory access due to cached memory index confusion
...
Note that on environments with wasm OOB trap handling enabled (generally speaking, 64bit platforms), it should prevent exploitation of this vulnerability assuming that its implementation is correct. 32bit chrome is exploitable in the same way as b/40061453.
...
O	O	O	(experimental)
O	X	O	(experimental)
X	O	X	x86-64 Linux/ChromeOS/Win/Mac, ARM64 Linux/ChromeOS/Mac (src)
X	X	O	Other Arch/OSes (32bit, Android, ...)
A question to the Chrome VRP team: Memory64 trap handling is now enabled by default but memory64 itself is still behind --experimental-wasm-memory64 flag, although it seems that there are plans to ship it soon. Would writing an exploit that runs on Chrome with memory64 enabled in 64bit architecture land me a Renderer RCE bonus? I can probably also write the exploit on 32bit or by disabling trap handler with --disable-features=WebAssemblyTrapHandler (simulating the behavior of Android) if that would conform to the VRP rules. <= not relevant anymore as the exploit is working in Android, see comment#6

P.S. Please use Seunghyun Lee (@0x10n) for reporter credit, thanks :)

Message last modified on Jul 8, 2024 09:02PM
se...@gmail.com<se...@gmail.com> #4Jul 6, 2024 10:07PM
...
[External Code]
Message last modified on Jul 7, 2024 04:05PM
se...@gmail.com<se...@gmail.com> #5Jul 7, 2024 02:08PM
Update:
...
When BoundsCheckMem() decides to use dynamic bounds check, the invariant end_offset <= min_size <= mem_size is broken and results in uintptr comparison with overflown effective_size. With memory64, this allows indexing of the full 64bit address space.
...
Message last modified on Jul 11, 2024 09:33PM
wasm-ic-memory-oob-v8sbx.js
74 KB
View
Download
se...@gmail.com<se...@gmail.com> #6Jul 8, 2024 03:21AM
Update:
...
We create multiple WebAssembly.Memory, shaping the layout such that we can OOB access the ArrayBuffer partition if a gap exists
Using this bug, probe memory out-of-bounds and find the WebAssembly.Memory that can OOB access the ArrayBuffer partition
...
OOB read to leak ArrayBuffer partition address as well as WebAssembly.Memory and the v8 sandbox base address
OOB write to overwrite next pointer at ArrayBuffer partition
At this step we already have arbitrary write due to b/40238514. In this exploit however, we use a different v8sbx exploit to reduce version offset hardcoding.
Spray PACKED_ELEMENTS arrays with mostly zeros
Reclaim the forged ArrayBuffer pointing into one of the PACKED_ELEMENTS arrays
Probe the arrays to find the one overlapping with the reclaim ArrayBuffer
This yields addrOf(), fakeObj() and almost arbitrary in-sandbox writes.
Use the above primitive to create an ArrayBuffer & DataView ranging the full v8 pointer cage region
...
Use b/350292240 to cause arbitrary WASM function signature confusion
This leads to v8sbx bypass, as well as leaking JIT address
Use the above primitive to overwrite JIT code to obtain RCE
The exploit calls prctl(PR_SET_NAME, "pwn") in both environments and has process continuation.
Message last modified on Jul 11, 2024 09:48PM
exp.html
102 KB
View
Download
wf...@chromium.org<wf...@chromium.org>Jul 8, 2024 11:54PM
Assigned to cl...@chromium.org.
sa...@chromium.org<sa...@chromium.org> #7Jul 10, 2024 02:50PM
...
jk...@chromium.org<jk...@chromium.org> #8Jul 11, 2024 09:29PM
Reassigned to jk...@chromium.org.
Great writeup, great bug. I'll upload a fix.

se...@gmail.com<se...@gmail.com> #9Jul 11, 2024 09:43PM
Update on comment#5: Seems that we can always satisfy the latter condition with Liftoff-generated atomic read/writes as it forces a (potentially broken) runtime check on atomic memory operations. Thus, having memory64 is enough to trigger the v8 sandbox bypass with in-sandbox exploit primitives. As memory64 is still technically an experimental feature I'm not opening a separate ticket on the v8 sandbox bypass, but feel free to do so if that would be better for tracking issues.

Attached is a PoC that triggers v8 sandbox bypass as explained in the lower section of comment#5. Run with --experimental-wasm-memory64 --sandbox-testing.

wasm-memory64-v8sbx.js
75 KB
View
Download
jk...@chromium.org<jk...@chromium.org> #10Jul 11, 2024 10:00PM
...
ap...@google.com<ap...@google.com> #11Jul 11, 2024 10:21PM
Marked as fixed.
Project: v8/v8
Branch: main
...
se...@gmail.com<se...@gmail.com> #12Jul 11, 2024 10:31PM
Re comment#10: Yes, the v8sbx bypass with race condition is an unrelated problem, it's just that the same invariant is broken with this bug too. I don't expect a VRP bounty on the v8sbx bypass as memory64 is experimental, but anyways split the issue out to b/352446085 :)

pe...@google.com<pe...@google.com> #13Jul 13, 2024 10:45PM
Setting milestone because of s0/s1 severity.

pe...@google.com<pe...@google.com> #14Jul 13, 2024 10:46PM
Setting Priority to P1 to match Severity s1. If this is incorrect, please reset the priority. The automation bot account won't make this change again.

pe...@google.com<pe...@google.com> #15Jul 13, 2024 10:53PM
...
jk...@chromium.org<jk...@chromium.org> #16Jul 15, 2024 03:48PM
#15:

https://chromium-review.googlesource.com/c/v8/v8/+/5695665
Yes, 128.0.6591.0
No
No
No
am...@chromium.org<am...@chromium.org> #17Jul 15, 2024 10:19PM
M126 and M127 merges approved for https://crrev.com/c/5695665 please merge this fix to 12.6 at your earliest convenience; please merge this fix to 12.7 as soon as possible, by 10am Pacific Tuesday, 16 July so this fix can be included in the M127 Stable RC being cut tomorrow for release next week

ap...@google.com<ap...@google.com> #18Jul 15, 2024 11:14PM
Project: v8/v8
...
ap...@google.com<ap...@google.com> #19Jul 15, 2024 11:14PM
Project: v8/v8
...
sp...@google.com<sp...@google.com> #20Jul 26, 2024 05:55AM
...
am...@chromium.org<am...@chromium.org> #21Jul 26, 2024 05:59AM
...
pe...@google.com<pe...@google.com> #22Sep 4, 2024 11:29PM
...
rz...@google.com<rz...@google.com> #23Sep 4, 2024 11:32PM
https://crrev.com/c/5832698
Low, no conflicts
126, 127
Yes
ap...@google.com<ap...@google.com> #24Sep 17, 2024 12:50AM
Project: v8/v8
...
pe...@google.com<pe...@google.com> #25Oct 19, 2024 01:37AM
