Title: V8 Sandbox Bypass: AAR/W via generic function table `call_indirect` rtt check bypass
# Headers
Reporter
...
https://chromium-review.googlesource.com/c/v8/v8/+/6072813
HW
--
Introduced In
--
...
Notably, we're using SBXCHECK(FunctionSigMatchesTable(...)) to check whether the function signature is in fact a canonical subtype of the actual table type. This check cannot be done statically with generic function typed tables and thus the checks are eventually done in runtime via rtt subtype checks. However, all the objects involved in the checks are within the v8 sandbox (rtt Map, WasmTypeInfo, ManagedObjectMaps, etc.) and are subject to corruption.
...
Comments
All comments
se...@gmail.com<se...@gmail.com> #2Jul 1, 2024 02:43PM
Update: Repro also working on latest v8 commit 362c1b605c7908bef1d6e48d2b2ee93f9eef23bf as expected.

se...@gmail.com<se...@gmail.com> #3Jul 1, 2024 03:00PM
Minor update: Cleared up some variable naming (ex: funcref_v_ls -> funcref_writer) as funcref objects are related to the function it references, not its type. Its maps however are directly related to the type (i.e. function signature, see CreateMapForType()), so the naming map_v_ls persists.

This does not affect the behavior of the PoC in any way.

rtt_subtype_check_bypass.js
2.6 KB
View
Download
ah...@google.com<ah...@google.com> #4Jul 1, 2024 08:19PM
...
Build a d8 binary with the given gn flags described in the above link
Run ./d8 --sandbox-testing ./rtt_subtype_check_bypass.js with both js files in the current directory
Observe the "V8 sandbox violation detected!" message.
cc jkummerow@ as the assignee of multiple v8sbx bypasses reported recently (b/348793147, b/349529650, b/349502157)

Message last modified on Jul 1, 2024 09:29PM
pe...@google.com<pe...@google.com> #7Jul 1, 2024 10:39PM
Setting Priority to P2 to match Severity s3. If this is incorrect, please reset the priority. The automation bot account won't make this change again.

is...@chromium.org<is...@chromium.org>Jul 2, 2024 03:43PM
Reassigned to sa...@chromium.org.
cl...@appspot.gserviceaccount.com<cl...@appspot.gserviceaccount.com> #8Jul 3, 2024 02:35PM
...
sa...@chromium.org<sa...@chromium.org> #9Jul 3, 2024 02:40PM
...
se...@gmail.com<se...@gmail.com> #10Jul 3, 2024 03:20PM
...
sa...@chromium.org<sa...@chromium.org> #11Jul 3, 2024 04:17PM
...
pe...@google.com<pe...@google.com> #12Oct 28, 2024 10:40PM
...
pe...@google.com<pe...@google.com> #13Nov 12, 2024 10:38PM
...
jk...@chromium.org<jk...@chromium.org> #14Nov 13, 2024 11:19PM
...
jk...@chromium.org<jk...@chromium.org> #15Nov 13, 2024 11:47PM
...
24...@project.gserviceaccount.com<24...@project.gserviceaccount.com> #16Nov 14, 2024 02:36PM
...
sa...@chromium.org<sa...@chromium.org> #17Nov 14, 2024 03:30PM
...
sa...@chromium.org<sa...@chromium.org> #18Nov 14, 2024 03:31PM
...
cl...@appspot.gserviceaccount.com<cl...@appspot.gserviceaccount.com> #19Nov 14, 2024 03:47PM
...
If you have any feedback on reproducing test cases, let us know at https://forms.gle/Yh3qCYFveHj6E5jz5 so we can improve.
se...@gmail.com<se...@gmail.com> #20Nov 15, 2024 01:33PM
Re #18: I was under the impression that WasmCodePointerTable is intended to block these signature confusion issues by (eventually) adding signature checks on the CPT entries, otherwise we will constantly have bypasses like b/379140430. I'm not sure about the overhead though...

sa...@chromium.org<sa...@chromium.org> #21Nov 15, 2024 05:22PM
...
There's a central "bottleneck" through which all (indirect) calls go that provides sandbox safety guarantees (in essence, that means fine-grained CFI). Something that's ~trivial to manually verify for correctness. Maybe similar to how we protect JS calls, where everything relies on the consistency of the JSDispatchTable, and there's a fairly straightforward CHECK for that.
...
We have hardware mechanisms in place that guarantee that JS or Wasm code cannot write outside the sandbox. Something like PKEYs, for example. See also this doc.
...
sr...@google.com<sr...@google.com>Nov 28, 2024 10:31PM
Accepted by sr...@google.com.
ap...@google.com<ap...@google.com> #22Dec 12, 2024 07:09PM
Marked as fixed.
Project: v8/v8
Branch: main
...
ap...@google.com<ap...@google.com> #23Dec 16, 2024 03:29PM
Project: v8/v8
Branch: main
...
ap...@google.com<ap...@google.com> #24Dec 18, 2024 10:52PM
Project: v8/v8
Branch: main
Author: Stephen Roettger <sroettger@google.com>
...
sp...@google.com<sp...@google.com> #25Dec 19, 2024 04:56AM
...
am...@chromium.org<am...@chromium.org> #26Dec 19, 2024 07:09AM
Congratulations Seunghyun! Thank you for this excellent report of another V8 sandbox bypass -- great work!

ap...@google.com<ap...@google.com> #27Dec 19, 2024 11:48AM
Project: v8/v8
Branch: main
Author: Lu Yahan <yahan@iscas.ac.cn>
...
pe...@google.com<pe...@google.com> #28Jan 2, 2025 07:43PM
Status: Assigned (reopened).
...
sr...@google.com<sr...@google.com>Jan 3, 2025 03:48PM
Marked as fixed.
pe...@google.com<pe...@google.com> #29Jan 3, 2025 03:59PM
Status: Assigned (reopened).
Dear owner, thanks for fixing this bug. We've reopened it because:
...
sr...@google.com<sr...@google.com>Jan 3, 2025 04:04PM
Marked as fixed.
ap...@google.com<ap...@google.com> #30Jan 14, 2025 11:07PM
Project: v8/v8
Branch: main
Author: Stephen Roettger <sroettger@google.com>
...
ap...@google.com<ap...@google.com> #31Jan 15, 2025 01:47AM
Project: v8/v8
Branch: main
Author: Stephen Roettger <sroettger@google.com>
...
ch...@google.com<ch...@google.com> #32Apr 12, 2025 07:38PM
