Title: use-after-poison write in WasmFunctionBuilder::WriteBody
...
https://chromium-review.googlesource.com/c/v8/v8/+/7594777
HW
--
Introduced In
62
...
# Description
Descriptionch...@google.com created issue on behalf of qy...@gmail.com #1
Feb 18, 2026 12:01AM
Steps to reproduce the problem
ran with: asan/d8 poc.js

Problem Description
The parser uses shared mutable state for heap-access shift recognition:
...
The crafted expression causes stale shift metadata to survive from a nested subexpression:
...
Heap-access validation then truncates generated wasm bytes to that stale position:
...
Metadata/body desynchronization occurs:
...
Serialization phase patches stale offsets without bounds checks:
...
Comments
All comments
cl...@appspot.gserviceaccount.com<cl...@appspot.gserviceaccount.com> #2Feb 18, 2026 01:50AM
...
ma...@google.com<ma...@google.com> #3Feb 18, 2026 01:52AM
Assigned to is...@chromium.org.
Security shepherd: assigning to V8 for triage

24...@project.gserviceaccount.com<24...@project.gserviceaccount.com> #4Feb 18, 2026 05:33AM
...
qy...@gmail.com<qy...@gmail.com> #5Feb 18, 2026 08:10AM
...
View
Download
ch...@google.com<ch...@google.com> #6Feb 18, 2026 07:35PM
Setting milestone because of s0/s1 severity.

ch...@google.com<ch...@google.com> #7Feb 18, 2026 07:35PM
Setting Priority to P1 to match Severity s1. If this is incorrect, please reset the priority. The automation bot account won't make this change again.

cl...@appspot.gserviceaccount.com<cl...@appspot.gserviceaccount.com> #8Feb 19, 2026 01:17AM
...
24...@project.gserviceaccount.com<24...@project.gserviceaccount.com> #9Feb 19, 2026 04:59AM
...
If you have any feedback on reproducing test cases, let us know at https://forms.gle/Yh3qCYFveHj6E5jz5 so we can improve.
is...@chromium.org<is...@chromium.org> #10Feb 19, 2026 07:36PM
Reassigned to fg...@google.com.
Thank you for the report.

Assigning to the author of the culprit CL for further investigation.

th...@chromium.org<th...@chromium.org>Feb 20, 2026 01:41AM
Reassigned to fg...@chromium.org.
fg...@chromium.org<fg...@chromium.org>Feb 20, 2026 03:51AM
Reassigned to cl...@google.com.
pe...@google.com<pe...@google.com>Feb 20, 2026 03:57AM
Reassigned to cl...@chromium.org.
jk...@chromium.org<jk...@chromium.org> #11Feb 20, 2026 09:05PM
...
qy...@gmail.com<qy...@gmail.com> #12Feb 21, 2026 09:33AM
...
HEAP32[a << (b >> 2) + ~~+g()] = c;
The parser records a stale heap_access_shift_position_ that later causes DeleteCodeAfter() to truncate the wasm function body without pruning the corresponding direct_calls_ vector. During serialization, WasmFunctionBuilder::WriteBody() iterates over all direct_calls_ entries — including the stale ones — and calls patch_u32v() at offsets that now point past the truncated body. This constitutes an out-of-bounds write with no bounds checking.

Detailed Code Flow
Step 1: Parsing + ~~+g() Emitting Calls and Recording direct_calls_
Back in the outer AdditiveExpression (the right operand of <<), the parser sees + and parses ~~+g(). For each ~~+g():

g() call: Emits kExprCallFunction (0x10) opcode, then calls EmitDirectCallIndex():
...
~~ (double bitwise NOT): Emits kExprI32AsmjsSConvertF64 (0xFA 0x59) — 2 bytes.
+ (addition): Emits kExprI32Add (0x6A) — 1 byte.
...
The Bug: body_.Truncate() simply moves the write pointer back. The direct_calls_ vector contains entries with offsets pointing into the now-deleted region which is never cleared.These stale entries survive and are used during serialization.

Step 3: Body Reconstruction
Final body size = 14 bytes. But direct_calls_ still contains entries with offsets like 8, 17, 26 ... (one per ~~+g() call), all recorded before truncation.
...
patch_u32v writes a 5-byte padded LEB128 at each stale offset with absolutely no bounds checking:
...
Each ~~+g() expression creates one stale direct_calls_ entry. The i-th entry (0-indexed) has:

call[i].offset = 8 + 9*i

Where:

8 = offset of the first placeholder in the pre-truncation body (after local.get a [2 bytes] + local.get b [2 bytes] + i32.const 2 [2 bytes] + i32.shr_s [1 byte] + call opcode [1 byte])
9 = bytes per call pattern (1 call opcode + 5 placeholder + 2 I32AsmjsSConvertF64 + 1 i32.add)
When we use different dummy numbers, call numbers, and g() indices, we can write data to different offsets. The four pocjs files below are proofs I used to demonstrate that data can be written to different offsets. Running them with the asan version of d8 shows that it writes values ​​to different offsets.

PoC file	Dummy num	call num	g index	write offset	Zone size
poc_addr_offset1144.js	10	103	10	1144	8192 B
poc_addr_offset3232.js	100	103	100	3227	8192 B
poc_addr_offset7360.js	200	200	200	7360	8192 B
poc_addr_offset8280.js	400	500	400	8280	24696 B
deleted
0 B
deleted
0 B
...
deleted
0 B
dx...@google.com<dx...@google.com> #13Feb 23, 2026 06:45PM
Marked as fixed.
Project: v8/v8
Branch:  main
...
[asm.js] Fix reset of heap_access_shift_position_

Expand for full commit details
ch...@google.com<ch...@google.com> #14Feb 23, 2026 07:42PM
...
jk...@chromium.org<jk...@chromium.org> #15Feb 23, 2026 07:45PM
#14:

https://chromium-review.googlesource.com/7594777
Not yet; check here: https://chromiumdash.appspot.com/commit/c0a41078e69f23668c8d34c61f286a1b5b211f19
No
No
No
24...@project.gserviceaccount.com<24...@project.gserviceaccount.com> #16Feb 24, 2026 03:45PM
...
ch...@google.com<ch...@google.com> #17Feb 24, 2026 07:00PM
...
ch...@google.com<ch...@google.com> #18Feb 24, 2026 07:00PM
...
ch...@google.com<ch...@google.com> #19Feb 24, 2026 07:00PM
...
jk...@chromium.org<jk...@chromium.org> #20Feb 24, 2026 07:45PM
#17/#18/#19: The amount of bot spam here is getting ridiculous. See #15.

Security fix
https://chromium-review.googlesource.com/7594777 (why don't you read that from the "Code Changes" field?)
Yes, 147.0.7701.0 (why don't you retrieve that information automatically via chromiumdash?)
No, bug is 8 years old
N/A
No
dr...@chromium.org<dr...@chromium.org> #21Feb 26, 2026 05:51AM
No crashes in Canary, merge approved to all three milestones.

dx...@google.com<dx...@google.com> #22Mar 3, 2026 12:49AM
Marked as fixed.
Project: v8/v8
...
Merged: [asm.js] Fix reset of heap_access_shift_position_
...
Hash: 361a5e1bee08e5cb8d37b676ff2cfb1c22b9af00
Date: Fri Feb 20 14:12:18 2026

dx...@google.com<dx...@google.com> #23Mar 3, 2026 03:08AM
Project: v8/v8
...
M src/asmjs/asm-parser.cc
A test/mjsunit/regress/wasm/regress-485152421.js
Hash: e6e9212e61783850e78548ed9ceef0e472d4e0c6
Date: Fri Feb 20 14:12:18 2026

pe...@google.com<pe...@google.com> #24Mar 3, 2026 03:12AM
LTS Milestone M144
...
dx...@google.com<dx...@google.com> #25Mar 3, 2026 05:04AM
Project: v8/v8
...
M src/asmjs/asm-parser.cc
A test/mjsunit/regress/wasm/regress-485152421.js
Hash: c7effa03ed998cbff58f21b1aa30dc63b6014f7a
Date: Fri Feb 20 14:12:18 2026

pe...@google.com<pe...@google.com> #26Mar 3, 2026 03:59PM
...
qk...@google.com<qk...@google.com> #27Mar 3, 2026 04:01PM
...
an...@google.com<an...@google.com> #28Mar 5, 2026 03:00AM
Approved for LTS-138

dx...@google.com<dx...@google.com> #29Mar 5, 2026 09:56PM
Project: v8/v8
Branch:  refs/branch-heads/13.8
Author:  Jakob Kummerow jkummerow@chromium.org
Link:    https://chromium-review.googlesource.com/7624862

[M138-LTS][asm.js] Fix reset of heap_access_shift_position_
...
M src/asmjs/asm-parser.cc
A test/mjsunit/regress/wasm/regress-485152421.js
Hash: 019035514b122fd21fde4c37ad7d63153c0c3f3b
Date: Fri Feb 20 14:12:18 2026

sp...@google.com<sp...@google.com> #30Mar 11, 2026 09:55PM
...
qk...@google.com<qk...@google.com> #31Apr 12, 2026 08:05PM
Labeled `LTS-Merge-Merged-144` because the patch was already merged to M144.
ch...@google.com<ch...@google.com> #32Jun 2, 2026 02:56PM
