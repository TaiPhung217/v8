...
let $s1_desc = $s1 + 1;
builder.addStruct({
  fields: [makeField(wasmRefNullType($src), false)],
  descriptor: $s1_desc,
});
builder.addStruct({
  fields: [],
...
let $sig_v_v = builder.addType(kSig_v_v);                               // conv_ref_cast_desc, conv_br_on_cast_desc
let $sig_s0_v = builder.addType(makeSig([], [wasmRefType($s0)]));       // conv_br_on_cast_desc blocktype
...
// ref.cast_desc unsoundness
builder.addFunction('conv_ref_cast_desc', $sig_v_v).addLocals(wasmRefType($s1_desc).exact(), 1).addBody([
  // instantiate g_src
  kGCPrefix, kExprStructNewDefault, $src,
  kExprGlobalSet, $g_src.index,
  
  // instantiate $s1_desc, $s1
  kExprGlobalGet, $g_src.index,
  kGCPrefix, kExprStructNewDefault, $s1_desc,
  kExprLocalTee, 0,
  kGCPrefix, kExprStructNew, $s1,

  // unsound cast to $s0
  kExprLocalGet, 0,
  kGCPrefix, kExprRefCastDesc, $s0,
...
// br_on_cast_desc* unsoundness
builder.addFunction('conv_br_on_cast_desc', $sig_v_v).addLocals(wasmRefType($s1_desc).exact(), 1).addBody([
  kExprBlock, $sig_s0_v,
    // instantiate g_src
    kGCPrefix, kExprStructNewDefault, $src,
    kExprGlobalSet, $g_src.index,
    ...
    // unsound cast to $s0
    kExprLocalGet, 0,
    ...wasmBrOnCastDesc(0, wasmRefType(kWasmAnyRef), wasmRefType($s0)),

    // cast should have worked
    kExprUnreachable,
  kExprEnd,
...
let {conv_ref_cast_desc, conv_br_on_cast_desc, addrof, fakeobj, caged_read, caged_write, caged_write_unsafe} = instance.exports;

// unsound cast in ref.cast_desc succeeds
conv_ref_cast_desc();

// unsound cast in br_on_cast_desc also work
conv_br_on_cast_desc();
